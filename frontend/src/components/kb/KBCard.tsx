import { ArrowUpRight, Calendar, Database, Pencil, Trash2 } from 'lucide-react';
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import type { KnowledgeBase } from '../../types';
import { Card } from '../ui/Card';
import { Spinner } from '../ui/Spinner';
import { UpdateKBModal } from './UpdateKBModal';
...
export const KBCard: React.FC<KBCardProps> = ({ kb, onDelete }) => {
  ...
  const [isUpdateKbModalOpen, setIsUpdateKbModalOpen] = useState(false);
  ...
  return (
    <>
          ...
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center space-x-1">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setIsUpdateKbModalOpen(true);
                }}
                disabled={isUpdateKbModalOpen}
      ...
      <UpdateKBModal
        kb={kb}
        isOpen={isUpdateKbModalOpen}
        onClose={() => setIsUpdateKbModalOpen(false)}
        onSuccess={() => setIsUpdateKbModalOpen(false)}
      />
